...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      scrolled ? "bg-gray-900/95 backdrop-blur-md shadow-2xl border-b border-gray-700" : "bg-transparent"
    }`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 bg-gray-900/95 backdrop-blur-md rounded-lg shadow-2xl border border-gray-700">
              {navItems.map((item) => (
                <a
                  key={item.href}
                  href={item.href}
